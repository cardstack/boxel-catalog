...
/* 
  Adventure Scenario (CardDef)
  - Standalone scenarios that can be re-used by Adventure cards
  - Keeps fields compatible with the V3/V4 embedded scenario structure
*/
...
import {
  ...
  field,
  contains,
  containsMany, // ¹ᵃ for tags/imageStyles/contentWarnings
} from 'https://cardstack.com/base/card-api'; // ¹ Core imports
import StringField from 'https://cardstack.com/base/string'; // ² Base fields
import BookOpenIcon from '@cardstack/boxel-icons/book-open'; // ³ Icon
...
export class AdventureScenario extends CardDef {
  ...
  // ⁶ Isolated format (simple, readable)
    ...
    <template>
      <article class='scenario-iso'>
        <header class='head'>
          ...
          <div class='meta'>
            {{#if (and @model.tags @model.tags.length)}}
              <div class='tags'>
            ...
            {{/if}}
            {{#if (and @model.imageStyles @model.imageStyles.length)}}
  ...
  // ⁸ Fitted format (badge/strip/tile/card patterns)
