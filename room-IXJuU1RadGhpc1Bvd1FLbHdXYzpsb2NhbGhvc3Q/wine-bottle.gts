...
export class WineBottle extends CardDef {
    ...
    <template>
      <article class='cellar-sheet' style={{htmlSafeAccent @model.liquidColor}}>
        <header class='eyebrow-row'>
          <p class='eyebrow'>CELLAR{{#if @model.region}} · {{@model.region}}{{/if}}</p>
          {{#if @model.wineType}}
        ...
        {{#if this.hasWindow}}
          <section class='timeline-row'>
            ...
            <div class='timeline'>
              ...
              >
                <span class='cursor-label'>today · {{this.windowLabel}}</span>
              </div>
            </div>
      ...
      <style scoped>
        .cellar-sheet {
          ...
          background:
            radial-gradient(ellipse at top, var(--cellar-bg-2) 0%, var(--cellar-bg) 70%);
        ...
        .type-red { background: rgba(122, 30, 42, 0.4); }
        .type-white { background: rgba(232, 226, 168, 0.18); }
        .type-rosé { background: rgba(244, 199, 194, 0.25); }
        .type-orange { background: rgba(216, 154, 79, 0.3); }
        .type-sparkling { background: rgba(232, 197, 71, 0.25); }
        ...
        @container cellar (inline-size <= 720px) {
          ...
          .glass-panel { justify-self: center; }
        }
        ...
        .wax-seal {
          ...
          border-radius: 50%;
          background:
            radial-gradient(circle at 35% 30%, #8a2434, var(--cellar-burgundy) 60%, #3d0e14);
          color: var(--cellar-cream);
          display: flex;
          flex-direction: column;
          align-items: center;
        ...
        .cursor-peak-window { color: var(--cellar-gold); }
        .cursor-before-window { color: #d89a4f; }
        .cursor-past-window { color: rgba(201, 169, 106, 0.4); }
        ...
        .delta-up { color: #7bc88a; }
        .delta-down { color: #d97a7a; }
        .delta-flat { color: var(--cellar-cream-dim); }
    ...
    <template>
      <article class='wine-card type-{{@model.wineType}}'>
        ...
        <div class='content'>
          ...
          <p class='sub'>{{#if @model.varietal}}<span>{{@model.varietal}}</span>{{/if}}{{#if @model.region}}<span class='region'>· {{@model.region}}</span>{{/if}}</p>
        </div>
        <div class='right'>
          {{#if @model.score}}
      ...
      <style scoped>
        ...
        .type-red    { border-left-color: #5a1a1f; }
        .type-white  { border-left-color: #c9b54a; }
        .type-rosé   { border-left-color: #e89aa0; }
        .type-orange { border-left-color: #b8732a; }
        .type-sparkling { border-left-color: #d4a83a; }
  ...
  static fitted = class Fitted extends Component<typeof WineBottle> {
    <template>
      <article class='fitted-bottle' style={{htmlSafeAccent @model.liquidColor}}>
        {{#if @model.label}}
          <div class='image-bg'>
      ...
      <style scoped>
        ...
        .fallback-bg {
          background:
            radial-gradient(ellipse at center, rgba(201, 169, 106, 0.15) 0%, transparent 70%);
        }
        .fallback-vintage {
