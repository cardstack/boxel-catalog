/* eslint-disable */ // Red Cab-Over Delivery Truck — procedural three.js model
...
function buildSculpture(THREE) {
  ...
  // ===== materials =====
  var MATERIALS = {};
  MATERIALS['m-red-gloss'] = new THREE.MeshPhysicalMaterial({
    color: new THREE.Color('#d81118'),
    roughness: 0.3,
    metalness: 0.1,
    clearcoat: 0.6,
  ...
  MATERIALS['m-red-satin'] = new THREE.MeshPhysicalMaterial({
    color: new THREE.Color('#d81118'),
    roughness: 0.55,
    metalness: 0.15,
    clearcoat: 0.15,
  ...
  MATERIALS['m-dark-steel'] = new THREE.MeshStandardMaterial({
    color: new THREE.Color('#1e2022'),
    roughness: 0.6,
    metalness: 0.7,
    envMapIntensity: 0.35
  });
  MATERIALS['m-grille'] = new THREE.MeshPhysicalMaterial({
  ...
  MATERIALS['m-glass'] = new THREE.MeshPhysicalMaterial({
    color: new THREE.Color('#dfeef5'),
    roughness: 0.05,
    metalness: 0.1,
    transparent: true,
    opacity: 0.85,
  ...
  MATERIALS['m-mirror'] = new THREE.MeshPhysicalMaterial({
  ...
  MATERIALS['m-rubber'] = new THREE.MeshPhysicalMaterial({
    color: new THREE.Color('#242628'),
    roughness: 0.9,
    metalness: 0,
    clearcoat: 0.15,
    clearcoatRoughness: 0.35,
  ...
  MATERIALS['m-silver-metal'] = new THREE.MeshStandardMaterial({
    color: new THREE.Color('#9fa5aa'),
    roughness: 0.35,
    metalness: 0.9,
    envMapIntensity: 0.35
  });
  MATERIALS['m-guard-white'] = new THREE.MeshPhysicalMaterial({
  ...
  MATERIALS['m-headlight'] = new THREE.MeshPhysicalMaterial({
    color: new THREE.Color('#f5f5f0'),
    roughness: 0.15,
    metalness: 0,
    emissive: new THREE.Color('#fff2c0'),
  ...
  MATERIALS['m-bezel'] = new THREE.MeshPhysicalMaterial({
    color: new THREE.Color('#1a1b1e'),
    roughness: 0.9,
    metalness: 0.1,
    clearcoat: 0.15,
  ...
  MATERIALS['m-brake-light'] = new THREE.MeshPhysicalMaterial({
    color: new THREE.Color('#e60000'),
    roughness: 0.2,
    metalness: 0.1,
  ...
  var FALLBACK_MATERIAL = new THREE.MeshStandardMaterial({
    color: 0x8a8f9c,
    roughness: 0.55,
    metalness: 0.25
  });
  ...
  function roundedRectShape(len, depth, r) {
    ...
    var hx = Math.max(len / 2 - r, 0.001),
      hz = Math.max(depth / 2 - r, 0.001);
  ...
  function buildGeometry(primitive, d) {
    switch (primitive) {
      ...
      case 'prism':
        {
          ...
          var length = Math.abs(d[0] ?? 1.5),
            span = Math.abs(d[1] ?? 1),
            height = Math.abs(d[2] ?? 0.6);
      ...
      case 'cylinder':
        return new THREE.CylinderGeometry(d[0] ?? 0.5, d[1] ?? d[0] ?? 0.5, d[2] ?? 1, Math.max(3, Math.round(d[3] ?? 48)));
      default:
        return null;
      // 'group' carries no geometry
    }
  }
  ...
  function addPart(p) {
    ...
    if (p.primitive === 'textDecal' && typeof buildTextDecal === 'function') {
      ...
      meshes.push(obj);
    } else if (p.primitive === 'curvedDecal' && typeof buildCurvedDecal === 'function') {
    ...
    objects.set(p.id, obj);
    parentIds.set(p.id, p.parent || null);
    if (p.attachTo) attachments.push({
      id: p.id,
      to: p.attachTo,
      prim: p.primitive
    });
    if (p.repeat) repeats.push({
      id: p.id,
      rep: p.repeat,
      to: p.attachTo
    });
  ...
  function assemble() {
    ...
    attachments.forEach(function (att) {
      ...
      ['x', 'y', 'z'].forEach(function (axis) {
        if (a.min[axis] > b.max[axis]) delta[axis] = b.max[axis] - a.min[axis] + margin;else if (a.max[axis] < b.min[axis]) delta[axis] = b.min[axis] - a.max[axis] + margin;
      });
    ...
    if (typeof seatSurfaceParts === 'function') {
      seatSurfaceParts(THREE, objects, attachments.filter(function (att) {
        return att.prim !== 'curvedDecal' && att.prim !== 'textDecal' && att.prim !== 'glow' && att.prim !== 'tube';
      }));
    }
    ...
    repeats.forEach(function (req) {
      ...
      if (!original) return;
      expandRepeatInstances(THREE, original, req.rep, req.to ? objects.get(req.to) : undefined, root, function (clone) {
    ...
    if (meshes.length > 1) {
      ...
      attachments.forEach(function (att) {
        attachToByName.set(att.id, att.to);
      });
    ...
    attachments.forEach(function (att) {
      ...
      var axes = ['x', 'y', 'z'];
      var normal = axes.reduce(function (a, b) {
        return pSize[b] < pSize[a] ? b : a;
      });
      var faceAxes = axes.filter(function (a) {
        return a !== normal;
      });
      ...
      var face = faceAxes.map(function (a) {
        return pSize[a];
      });
      var plateLike = pSize[normal] < 0.3 * Math.min(face[0], face[1]);
      var thin = pSize[normal] < wSize[normal] * 0.6;
      var fits = faceAxes.every(function (a) {
        return pSize[a] <= wSize[a] * 1.1;
      });
  ...
  addPart({
    ...
    mat: 'm-dark-steel',
    attachTo: 'tank-left'
  }); // structural beam under cab and box
  addPart({
    id: 'main-cab',
    parent: 'root',
    primitive: 'roundedBox',
    d: [1, 1.3, 1.3, 0.08, 0.03],
    pos: [0, 0.93, -1.1],
    mat: 'm-red-gloss',
    attachTo: 'chassis-frame'
  }); // hollow driver cab shell
  addPart({
    id: 'roof-fairing',
    parent: 'root',
    primitive: 'prism',
    ...
    mat: 'm-red-gloss',
    attachTo: 'main-cab'
  }); // aero fairing tapering to cargo box
  addPart({
    id: 'cargo-box',
    parent: 'root',
    primitive: 'box',
  ...
  addPart({
    ...
    parent: 'root',
    primitive: 'roundedBox',
    d: [1, 0.5, 0.35, 0.05, 0.02],
    pos: [0, 0.61, -1.88],
    mat: 'm-red-gloss',
    attachTo: 'main-cab'
  }); // lower front housing headlights
  addPart({
    id: 'headlight-left',
    parent: 'root',
    primitive: 'box',
    ...
    pos: [-0.32, 0.66, -2.05],
    mat: 'm-headlight',
    attachTo: 'front-bumper'
  }); // multi-lens headlight cluster
  addPart({
    id: 'headlight-right',
    parent: 'root',
    primitive: 'box',
  ...
  addPart({
    ...
    parent: 'root',
    primitive: 'roundedBox',
    d: [0.12, 0.35, 0.15, 0.02, 0.01],
    pos: [-0.54, 1.35, -1.55],
    mat: 'm-mirror',
    attachTo: 'main-cab'
  }); // tall driver side mirror housing
  addPart({
    id: 'front-wheel-left',
    parent: 'root',
    primitive: 'cylinder',
  ...
  addPart({
    ...
    rot: [0, 0, 1.5708],
    mat: 'm-silver-metal',
    attachTo: 'front-wheel-left'
  }); // silver hub recessed in tire
  addPart({
    id: 'front-wheel-rim-right',
    parent: 'root',
    primitive: 'cylinder',
    d: [0.22, 0.22, 0.05, 16],
    pos: [0.565, 0.42, -1],
  ...
  addPart({
    ...
    mat: 'm-mirror',
    attachTo: 'main-cab'
  }); // tall passenger side mirror housing, mirrored counterpart
  addPart({
    id: 'windshield-block',
    parent: 'root',
    primitive: 'box',
    ...
    mat: 'm-glass',
    attachTo: 'main-cab'
  }); // large rectangular black glass windshield block
  addPart({
    id: 'rear-wheel-rim-left',
    parent: 'root',
    primitive: 'cylinder',
    d: [0.22, 0.22, 0.05, 16],
    pos: [-0.565, 0.42, 1.3],
    rot: [0, 0, 1.5708],
    mat: 'm-silver-metal',
    attachTo: 'rear-wheel-left'
  }); // silver hub recessed in tire
  addPart({
    id: 'rear-wheel-rim-right',
    parent: 'root',
    primitive: 'cylinder',
    d: [0.22, 0.22, 0.05, 16],
    pos: [0.565, 0.42, 1.3],
  ...
  addPart({
    ...
    mat: 'm-dark-steel',
    attachTo: 'chassis-frame'
  }); // rear bumper bar mounted to chassis
  addPart({
    id: 'brake-light-left',
    parent: 'root',
    primitive: 'box',
    ...
    pos: [-0.3, 0.45, 1.75],
    mat: 'm-brake-light',
    attachTo: 'rear-bumper'
  }); // left red brake light lens
  addPart({
    ...
    mat: 'm-brake-light',
    attachTo: 'rear-bumper'
  }); // right red brake light lens
  addPart({
    id: 'mudguard-left',
    parent: 'root',
    primitive: 'roundedBox',
    d: [0.55, 0.2, 0.9, 0.1, 3],
    pos: [-0.75, 0.85, 1.3],
    mat: 'm-dark-steel',
    attachTo: 'chassis-frame'
  }); // protective curved mudguard arch above left rear wheels
  addPart({
    id: 'mudguard-right',
    parent: 'root',
    primitive: 'roundedBox',
    ...
    mat: 'm-dark-steel',
    attachTo: 'chassis-frame'
  }); // protective curved mudguard arch above right rear wheels
  assemble();
  return {
    group: root,
    meshes: meshes
...
var SCULPT_SPEC = {
  "objectName": "Red Cab-Over Delivery Truck",
  "inputKind": "object",
  "objectClass": "hard-surface",
  "complexity": "complex",
  "identityFeatures": ["bright red cab-over truck body", "aerodynamic roof fairing spoiler above driver cab", "large rectangular red cargo box with lower curtain trim", "tiered front grille with horizontal dark slats", "integrated multi-lens headlight clusters on front bumper", "side safety guard rails and cylindrical silver fuel tanks", "dual rear wheels and front steer wheels with silver rims"],
  "materials": [{
    "materialId": "m-red-gloss",
    "baseColor": "#d81118",
    "roughness": 0.3,
    "metalness": 0.1,
    "opacity": null,
  ...
  "components": [{
    ...
    "nodeId": "chassis-frame",
    "parentId": "root",
    "primitive": "box",
    "dimensions": [0.9, 0.3, 3],
    "position": [0, 0.5, 0.2],
    "rotation": [0, 0, 0],
    "scale": [1, 1, 1],
    "materialId": "m-dark-steel",
    "text": null,
    "partRef": "chassis-frame",
    "textureRef": null,
    "textureUrl": null,
    "repeat": null,
    "attachTo": "tank-left",
    "note": "structural beam under cab and box"
  }, {
    "nodeId": "main-cab",
    "parentId": "root",
    "primitive": "roundedBox",
    "dimensions": [1, 1.3, 1.3, 0.08, 0.03],
    "position": [0, 0.93, -1.1],
    "rotation": [0, 0, 0],
    "scale": [1, 1, 1],
    "materialId": "m-red-gloss",
    ...
    "repeat": null,
    "attachTo": "chassis-frame",
    "note": "hollow driver cab shell"
  }, {
    "nodeId": "roof-fairing",
    "parentId": "root",
    "primitive": "prism",
    "dimensions": [0.95, 0.9, 0.35],
    "position": [0, 1.88, -0.75],
    "rotation": [0, 0, 0],
    "scale": [1, 1, 1],
    "materialId": "m-red-gloss",
    "text": null,
    "partRef": "roof-fairing",
    "textureRef": null,
    "textureUrl": null,
    "repeat": null,
    "attachTo": "main-cab",
    "note": "aero fairing tapering to cargo box"
  }, {
    "nodeId": "cargo-box",
    "parentId": "root",
    "primitive": "box",
    "dimensions": [1.05, 1.5, 1.9],
    "position": [0, 1.45, 0.47],
    "rotation": [0, 0, 0],
    "scale": [1, 1, 1],
    "materialId": "m-red-satin",
    "text": null,
    ...
    "parentId": "root",
    "primitive": "roundedBox",
    "dimensions": [1, 0.5, 0.35, 0.05, 0.02],
    "position": [0, 0.61, -1.88],
    "rotation": [0, 0, 0],
    "scale": [1, 1, 1],
    "materialId": "m-red-gloss",
    "text": null,
    "partRef": "front-bumper",
    "textureRef": null,
    "textureUrl": null,
    "repeat": null,
    "attachTo": "main-cab",
    "note": "lower front housing headlights"
  }, {
    "nodeId": "headlight-left",
    "parentId": "root",
    "primitive": "box",
    "dimensions": [0.18, 0.12, 0.05],
    "position": [-0.32, 0.66, -2.05],
    "rotation": [0, 0, 0],
    "scale": [1, 1, 1],
    "materialId": "m-headlight",
    "text": null,
    ...
    "repeat": null,
    "attachTo": "front-bumper",
    "note": "multi-lens headlight cluster"
  }, {
    "nodeId": "headlight-right",
    "parentId": "root",
    "primitive": "box",
    "dimensions": [0.18, 0.12, 0.05],
    "position": [0.32, 0.66, -2.05],
    ...
    "parentId": "root",
    "primitive": "box",
    "dimensions": [0.5, 0.35, 0.08],
    "position": [0, 0.82, -1.68],
    "rotation": [0, 0, 0],
    "scale": [1, 1, 1],
    "materialId": "m-grille",
    "text": null,
    "partRef": "front-grille",
    "textureRef": null,
    "textureUrl": null,
    "repeat": null,
    "attachTo": "main-cab",
    "note": "tiered louvered grille slats"
  }, {
    "nodeId": "side-window-left",
    "parentId": "root",
    "primitive": "box",
    "dimensions": [0.05, 0.4, 0.5],
    "position": [-0.445, 1.22, -1],
    "rotation": [0, 0, 0],
    "scale": [1, 1, 1],
    "materialId": "m-glass",
    "text": null,
    ...
    "repeat": null,
    "attachTo": "main-cab",
    "note": "driver side window"
  }, {
    "nodeId": "side-window-right",
    "parentId": "root",
    "primitive": "box",
    "dimensions": [0.05, 0.4, 0.5],
    "position": [0.445, 1.22, -1],
    ...
    "repeat": null,
    "attachTo": "main-cab",
    "note": "passenger side window"
  }, {
    "nodeId": "side-mirror",
    "parentId": "root",
    "primitive": "roundedBox",
    "dimensions": [0.12, 0.35, 0.15, 0.02, 0.01],
    "position": [-0.54, 1.35, -1.55],
    "rotation": [0, 0, 0],
    "scale": [1, 1, 1],
    "materialId": "m-mirror",
    "text": null,
    "partRef": "side-mirrors",
    "textureRef": null,
    "textureUrl": null,
    "repeat": null,
    "attachTo": "main-cab",
    "note": "tall driver side mirror housing"
  }, {
    "nodeId": "front-wheel-left",
    "parentId": "root",
    "primitive": "cylinder",
    "dimensions": [0.42, 0.42, 0.3, 20],
    "position": [-0.47, 0.42, -1],
    "rotation": [0, 0, 1.5708],
    "scale": [1, 1, 1],
    "materialId": "m-rubber",
    "text": null,
    "partRef": "front-wheels",
    "textureRef": null,
    "textureUrl": null,
    "repeat": null,
    "attachTo": "chassis-frame",
    "note": "front steer tire"
  }, {
    "nodeId": "front-wheel-right",
    "parentId": "root",
    "primitive": "cylinder",
    "dimensions": [0.42, 0.42, 0.3, 20],
    "position": [0.47, 0.42, -1],
    ...
    "nodeId": "front-wheel-rim-left",
    "parentId": "root",
    "primitive": "cylinder",
    "dimensions": [0.22, 0.22, 0.05, 16],
    "position": [-0.565, 0.42, -1],
    "rotation": [0, 0, 1.5708],
    "scale": [1, 1, 1],
    "materialId": "m-silver-metal",
    "text": null,
    ...
    "repeat": null,
    "attachTo": "front-wheel-left",
    "note": "silver hub recessed in tire"
  }, {
    "nodeId": "front-wheel-rim-right",
    "parentId": "root",
    "primitive": "cylinder",
    "dimensions": [0.22, 0.22, 0.05, 16],
    "position": [0.565, 0.42, -1],
    ...
    "nodeId": "rear-wheel-left",
    "parentId": "root",
    "primitive": "cylinder",
    "dimensions": [0.42, 0.42, 0.3, 20],
    "position": [-0.2, 0.42, 1.3],
    "rotation": [0, 0, 1.5708],
    "scale": [1, 1, 1],
    "materialId": "m-rubber",
    "text": null,
    ...
    "attachTo": "chassis-frame",
    "note": "dual rear tire pair"
  }, {
    "nodeId": "rear-wheel-right",
    "parentId": "root",
    "primitive": "cylinder",
    "dimensions": [0.42, 0.42, 0.3, 20],
    "position": [0.2, 0.42, 1.3],
    ...
    "parentId": "root",
    "primitive": "cylinder",
    "dimensions": [0.18, 0.18, 0.9, 16],
    "position": [-0.42, 0.6233, 0.15],
    "rotation": [1.5708, 0, 0],
    "scale": [1, 1, 1],
    "materialId": "m-silver-metal",
    "text": null,
    "partRef": "underbody-guards-and-tanks",
    "textureRef": null,
    "textureUrl": null,
    "repeat": null,
    "attachTo": "chassis-frame",
    "note": "aluminum fuel tank"
  }, {
    "nodeId": "tank-right",
    "parentId": "root",
    "primitive": "cylinder",
    ...
    "parentId": "root",
    "primitive": "box",
    "dimensions": [0.06, 0.15, 1.6],
    "position": [-0.48, 0.55, 0.15],
    "rotation": [0, 0, 0],
    "scale": [1, 1, 1],
    "materialId": "m-guard-white",
    "text": null,
    ...
    "repeat": null,
    "attachTo": "main-cab",
    "note": "tall passenger side mirror housing, mirrored counterpart"
  }, {
    "nodeId": "windshield-block",
    "parentId": "root",
    "primitive": "box",
    "dimensions": [0.98, 0.9, 0.1],
    ...
    "repeat": null,
    "attachTo": "main-cab",
    "note": "large rectangular black glass windshield block"
  }, {
    "nodeId": "rear-wheel-rim-left",
    "parentId": "root",
    "primitive": "cylinder",
    "dimensions": [0.22, 0.22, 0.05, 16],
    "position": [-0.565, 0.42, 1.3],
    "rotation": [0, 0, 1.5708],
    "scale": [1, 1, 1],
    "materialId": "m-silver-metal",
    "text": null,
    ...
    "repeat": null,
    "attachTo": "chassis-frame",
    "note": "rear bumper bar mounted to chassis"
  }, {
    "nodeId": "brake-light-left",
    "parentId": "root",
    "primitive": "box",
    "dimensions": [0.15, 0.06, 0.02],
    "position": [-0.3, 0.45, 1.75],
    "rotation": [0, 0, 0],
    "scale": [1, 1, 1],
    "materialId": "m-brake-light",
    "text": null,
    ...
    "repeat": null,
    "attachTo": "rear-bumper",
    "note": "left red brake light lens"
  }, {
    "nodeId": "brake-light-right",
    "parentId": "root",
    "primitive": "box",
    "dimensions": [0.15, 0.06, 0.02],
    "position": [0.3, 0.45, 1.75],
    ...
    "repeat": null,
    "attachTo": "rear-bumper",
    "note": "right red brake light lens"
  }, {
    "nodeId": "mudguard-left",
    "parentId": "root",
    "primitive": "roundedBox",
    "dimensions": [0.55, 0.2, 0.9, 0.1, 3],
    "position": [-0.75, 0.85, 1.3],
    "rotation": [0, 0, 0],
    "scale": [1, 1, 1],
    "materialId": "m-dark-steel",
    "text": null,
    ...
    "repeat": null,
    "attachTo": "chassis-frame",
    "note": "protective curved mudguard arch above left rear wheels"
  }, {
    "nodeId": "mudguard-right",
    "parentId": "root",
    "primitive": "roundedBox",
    "dimensions": [0.55, 0.2, 0.9, 0.1, 3],
    "position": [0.75, 0.85, 1.3],
...
if (typeof module !== 'undefined') {
  module.exports = {
    buildSculpture: buildSculpture,
    SCULPT_SPEC: SCULPT_SPEC
  };
}