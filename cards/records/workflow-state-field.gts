...
/**
 * One state in a data-driven workflow. Unlike a fixed status enum (see
 * `status-field.gts`, which owns the STATIC case), these are rows in a
 ...
 * states are where Archive becomes legal, `initial` is where minting happens.
 */
