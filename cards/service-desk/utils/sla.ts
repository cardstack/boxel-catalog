...
/** Nearest breach first over any list, given how to read each item's timer facts. */
export function sortByUrgency<T>(
  items: T[],
  factsOf: (item: T) => TimerFacts | undefined = (item) =>
    item as unknown as TimerFacts,
  now: Date = new Date(),
): T[] {
  return [...items].sort(
    (a, b) =>
      urgencyRank(timerSnapshot(factsOf(a) ?? {}, now)) -
      urgencyRank(timerSnapshot(factsOf(b) ?? {}, now)),
  );
}
