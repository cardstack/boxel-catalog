 ...
 *
 * WHY NOT `Task`. The catalog Task was considered and rejected: a task
...
export class Obligation extends CardDef {
  ...
  static edit = class Edit extends Component<typeof Obligation> {
    ...
    goTo = (id: string, event: Event) => {
      ...
      let root = (event.currentTarget as HTMLElement).closest(
        '.obligation-edit',
      );
      root
        ...
        <div class='edit-body'>
          ...
          <div class='sects'>
            ...
            >
              ...
              <FieldContainer
                @label='Consequence of missing it'
                @vertical={{true}}
              >
                <@fields.consequence />
              </FieldContainer>
            ...
            >
              <h3>Schedule
                <span class='sect-hint'>next due date and status are computed
                  from these</span></h3>
              <div class='row cols-2'>
            ...
            <section
              class='sect
                {{if (eq this.activeSection "completions") "focused"}}'
              data-sect='completions'
            >
              <h3>Completions
                <span class='sect-hint'>one entry per period fulfilled, with
                  evidence</span></h3>
  ...
  static isolated = class Isolated extends Component<typeof Obligation> {
    ...
    get countdown(): { n: string; unit: string } {
      ...
      if (this.args.model?.status === 'closed')
        return { n: '—', unit: 'closed' };
      if (typeof d !== 'number') return { n: '—', unit: 'no schedule' };
      if (d < 0)
        return {
          n: `${Math.abs(d)}`,
          unit: Math.abs(d) === 1 ? 'day overdue' : 'days overdue',
        };
  ...
  static fitted = class Fitted extends Component<typeof Obligation> {
    ...
    <template>
      <article class='fit'>
        ...
        <footer class='r-meta'><span
            class='val'
          >{{@model.nextDueDate}}</span></footer>
      </article>
      <style scoped>
        .fit {
          --type-ratio: 1.24;
