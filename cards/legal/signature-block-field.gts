...
export class SignatureBlockField extends FieldDef {
  ...
  @field displayTitle = contains(StringField, {
    computeVia: function (this: SignatureBlockField) {
      let title =
        this.signatory?.signingTitle?.trim() || this.signerTitle?.trim() || '';
      // When the title is already standing in for the name, do not print it twice.
      let name =
        this.signatory?.person?.name?.trim() || this.signerName?.trim();
      return name ? title : '';
    },
  });
  ...
  static atom = class Atom extends Component<typeof this> {
    get hue(): Hue {
      return (
        SIGNATURE_BLOCK_STATUS_HUE[this.args.model?.lineStatus ?? 'pending'] ??
        'slate'
      );
    }
    <template>
      ...
      <div class='sb {{if (eqStatus @model.lineStatus "signed") "is-signed"}}'>
        <div class='sb-head'>
          ...
          <span class='sb-party'>{{if
            ...
            {{#if @model.party.roleLabel}}<span
                class='sb-role'
              >({{@model.party.roleLabel}})</span>{{/if}}</span>
        </div>
        <p class='sb-signer'>
          <span class='sb-name'>{{this.signerName}}</span>{{~#if
            this.signerTitle
          }}<span class='sb-title'>, {{this.signerTitle}}</span>{{/if}}
        </p>
        <div class='sb-foot'>
          <StatePill
            @label={{signatureBlockStatusLabel @model.lineStatus}}
 ...
 */
SignatureBlockField.edit = class Edit extends Component<
  typeof SignatureBlockField
> {
  <template>
    <div class='sb-edit'>
      ...
      <div class='sb-who'>
        <FieldContainer
          @label='Our signatory (authority is checked)'
          @vertical={{true}}
        >
          <@fields.signatory />
        </FieldContainer>
      ...
      <FieldContainer
        @label='Signature reference (envelope id or checksum)'
        @vertical={{true}}
      >
        <@fields.signatureRef />
      </FieldContainer>
...
export function ceremonyState(
  ...
  if (
    list.some((b) => b.lineStatus === 'signed' || b.lineStatus === 'requested')
  )
    return 'in progress';
  return 'not started';
}
  ...
  for (let b of list) {
    ...
    if (!b.party?.entity) {
      findings.push({
        ...
        level: 'block',
        message:
          'No legal entity behind this line — a signature binds a party, not a person.',
      });
    }
    ...
    if (b.lineStatus === 'signed') {
      if (!b.signatureRef?.trim()) {
        findings.push({
          ...
          level: 'block',
          message:
            'Marked signed but carries no signature reference — nothing proves it.',
        });
      }
      ...
      findings.push({
        ...
        level: 'block',
        message:
          'A later line signed while this one is still open — out of signing order.',
      });
    }
  }
