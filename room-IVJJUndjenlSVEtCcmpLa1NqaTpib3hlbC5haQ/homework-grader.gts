...
const bgImageUrl: string = new URL(
  './homework-grader-bg.png',
  // @ts-ignore TS1470 .gts is treated as CommonJS here even though this asset URL is valid at runtime
  import.meta.url,
).href;
...
class HomeworkIsolated extends Component<typeof Homework> {
  ...
  get currentAnswersSnapshot() {
    return JSON.stringify(
      this.args.model.questions?.map((q) => q.answer) ?? [],
    );
  }
    ...
    <article class='retro-fitted {{this.gradeClass}}'>
      ...
      <article class='card-fmt'>
        ...
        <section class='card-body'>
          <img src={{bgImageUrl}} alt='' class='card-bg' aria-hidden='true' />
...
export class HomeworkGrader extends CardDef {
  static displayName = 'HomeworkGrader';
