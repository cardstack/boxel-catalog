...
import { isCardInstance } from '@cardstack/runtime-common/code-ref';
import { logger } from '@cardstack/runtime-common/log';
import { RealmPaths } from '@cardstack/runtime-common/paths';
import {
  extractRelationshipIds,
  type Relationship,
} from '@cardstack/runtime-common/resource-types';
...
type LooseSingleCardDocumentLike = {
  data: {
    id?: string;
...
> {
  ...
  private async expandInstances(instances: CardDef[]): Promise<CardDef[]> {
    ...
    while (queue.length > 0) {
      ...
      const serialized = serializedResult.json as LooseSingleCardDocumentLike;
