...
export function noticeDeadlineOf(
  ...
  let days =
    typeof noticeDays === 'number' && Number.isFinite(noticeDays)
      ? noticeDays
      : 0;
...
export class EffectivePeriodField extends FieldDef {
  ...
  static atom = class Atom extends Component<typeof this> {
    ...
    get hue(): Hue {
      return (
        PERIOD_STATE_HUE[
          (this.args.model?.periodState as PeriodState) ?? 'in force'
        ] ?? 'slate'
      );
    }
    <template>
  ...
  static embedded = class Embedded extends Component<typeof this> {
    ...
    get noticeLine(): string | undefined {
      ...
      if (d === undefined || d === null)
        return 'Auto-renews; notice period not set.';
      if (d < 0)
        return `Notice deadline passed ${Math.abs(d)}d ago — renews for ${m.renewalTermMonths ?? '?'} months.`;
    ...
    <template>
      <div class='ep'>
        ...
        {{#if @model.autoRenews}}
          <div
            class='ep-row ep-notice
              {{if (eqState this.state "notice passed") "is-late"}}
 ...
 */
EffectivePeriodField.edit = class Edit extends Component<
  typeof EffectivePeriodField
> {
  <template>
    <div class='ep-edit'>
      <div class='ep-dates'>
      ...
      <div class='ep-renew'>
        ...
        <FieldContainer
          @label='Notice period (days before end)'
          @vertical={{true}}
        >
          <@fields.noticeDays />
        </FieldContainer>
