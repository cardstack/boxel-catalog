...
import SliderField, { type SliderFieldConfiguration } from '../slider';

const demoConfig: SliderFieldConfiguration = {
  presentation: 'slider',
  options: { min: 0, max: 100, suffix: '%', showValue: true },
};

const usageCode = `@field level = contains(SliderField);`;
const configCode = `const demoConfig: SliderFieldConfiguration = {
  presentation: 'slider',
  options: { min: 0, max: 100, suffix: '%', showValue: true },
};

@field configuredSlider = contains(SliderField, {
  configuration: demoConfig,
});`;

export class SliderExample extends CardDef {
  static displayName = 'Slider Example';

  @field level = contains(SliderField);
  @field configuredSlider = contains(SliderField, {
    configuration: demoConfig,
  });
  ...
  static isolated = class Isolated extends Component<typeof this> {
    apiRows = [
      { name: 'min', type: 'number', default: '0', desc: 'Lowest value.' },
      { name: 'max', type: 'number', default: '100', desc: 'Highest value.' },
      {
        name: 'decimals',
        type: 'number',
        default: '—',
        desc: 'Decimal places displayed.',
      },
      {
        name: 'prefix',
        type: 'string',
        default: '—',
        desc: 'Text shown before the value.',
      },
      {
        name: 'suffix',
        type: 'string',
        default: '—',
        desc: 'Text shown after the value.',
      },
      {
        name: 'showValue',
        type: 'boolean',
        default: '—',
        desc: 'Display the numeric value.',
      },
    ];
    ...
    <template>
      <article class='field-example'>
        ...
        <section class='fe-panel'>
          ...
          <div class='fe-panel-body'>
            <@fields.configuredSlider @format='edit' />
          </div>
        ...
        <section class='fe-panel'>
          ...
          <p class='fe-api-intro'>Typed by SliderFieldConfiguration, exported by
            the field.</p>
          <div class='fe-api'>
