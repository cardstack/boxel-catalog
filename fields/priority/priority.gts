 ...
 * for different reasons and by different people, and merging them produces
 ...
 * is the most common thing anyone does with this field, and a string sort puts
 * P1 next to P4 and Critical after Low.
 */
export interface PriorityOption {
  ...
  rank?: number;
  /** Multiplies a target duration; a service desk uses it for SLA maths. */
  factor?: number;
  /**
   * One sentence on when this level applies, in the reader's words. The block
   * has no opinion on what P1 means; the consumer supplies it.
   */
  meaning?: string;
}
...
export interface PriorityFieldConfig {
  ...
  icon?: unknown;
  /**
   * The atom is the short form used in dense rows. When false the atom shows
   * the full label instead of the bare value.
   */
  shortAtom?: boolean;
}
...
export function priorityField(config: PriorityFieldConfig) {
  ...
  class Priority extends Base {
    ...
    // The top priority is the one state allowed a solid fill, so the eye lands
    // on the P1s in a queue without reading a word.
...
function priorityRankOf(
  ...
  if (idx === -1) {
    // Unset sorts last: a record nobody has triaged should not outrank one
    // somebody deliberately marked critical.
