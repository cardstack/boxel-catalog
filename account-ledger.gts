...
import {
  ...
  field,
  contains,
  linksToMany, // ¹ LedgerEntry is CardDef, requires linksToMany
  Component,
...
export class AccountLedger extends CardDef { // ¹⁰
  static displayName = 'Account Ledger';
  ...
  @field entries = linksToMany(LedgerEntry); // ¹⁵ LedgerEntry is CardDef, requires linksToMany

  @field cardTitle = contains(StringField, { // ¹⁶
    computeVia: function (this: AccountLedger) {
      return this.accountName ?? 'Account Ledger';
    },
  });

  // ¹⁷ Isolated - full ledger with entry creation
  static isolated = class Isolated extends Component<typeof AccountLedger> {
    ...
    // ¹⁹ CREATE ENTRY - Use new LedgerEntry() for proper CardDef instance
    addEntry = () => {
      ...
      try {
        // THE FIX: Create a proper LedgerEntry instance, NOT a plain object
        ...
        newEntry.entryDate = new Date(); // DateField expects Date object
      ...
      <article class="ledger">
        <header class="ledger-header">
          <div class="account-info">
            <h1>{{if @model.accountName @model.accountName "Account Ledger"}}</h1>
    ...
    get isCredit() {
      return this.newType === 'credit';
    }

    get isDebit() {
      return this.newType === 'debit';
    }
  };

  // ²¹ Embedded view
  static embedded = class Embedded extends Component<typeof AccountLedger> {
    get balance() {
  ...
  // ²² Fitted view
  static fitted = class Fitted extends Component<typeof AccountLedger> {
    get balance() {
      let bal = this.args.model?.openingBalance ?? 0;
      for (const entry of this.args.model?.entries ?? []) {
        bal += (entry.credit ?? 0) - (entry.debit ?? 0);
